...
import { isDateWithinLeaveRequest } from "./staffService";
...
export function getLeaveForDate(requests, dateString, statuses = ["Approved"]) {
  ...
  return safeRequests.filter(
    (request) =>
      isDateWithinLeaveRequest(request, dateString) &&
...
export function assessLeaveRequestCover({ request, staffList = staff, requests = [] }) {
  const requestDate = request.date || request.startDate;
...
export function assessLeaveRequestRangeCover({ request, staffList = staff, requests = [], dates = [] }) {
  const safeDates = dates.length > 0
    ? dates
    : [request.startDate || request.date].filter(Boolean);

  const snapshots = safeDates.map((date) =>
    assessCoverForDate({
      date,
      staffList,
      requests: requests.filter((item) => item.id !== request.id),
  ...
  const riskiestSnapshot = snapshots.reduce(
    (highest, snapshot) => (snapshot.riskScore > highest.riskScore ? snapshot : highest),
    snapshots[0] || assessLeaveRequestCover({ request, staffList, requests })
  );

  return {
    ...riskiestSnapshot,
    snapshots,
    warnings: snapshots.flatMap((snapshot) =>
      snapshot.warnings.map((warning) => ({ ...warning, date: snapshot.date, formattedDate: snapshot.formattedDate }))
    ),
    availableStaff: riskiestSnapshot?.availableStaff || [],
  };
}
...
export function getCoverMetrics({ requests = [], staffList = staff } = {}) {
  ...
  const affectedDates = [
    ...new Set(
      requestsWithRisk.flatMap((request) => {
